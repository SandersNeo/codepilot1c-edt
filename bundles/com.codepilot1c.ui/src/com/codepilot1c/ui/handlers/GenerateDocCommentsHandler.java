...
/**
 * Handler for generating documentation comments for selected method.
 *
 * <p>Analyzes the selected method/procedure and generates appropriate
 * documentation comments in 1C BSL format.</p>
 *
 * <p>Future enhancement: Insert the doc comments directly above the method.</p>
 */
public class GenerateDocCommentsHandler extends AbstractHandler {

    private static final VibeLogger.CategoryLogger LOG = VibeLogger.forClass(GenerateDocCommentsHandler.class);

    private static final String DOC_COMMENTS_PROMPT_TEMPLATE =
            "Generate documentation comments for the following 1C BSL method/procedure. " //$NON-NLS-1$
            + "Use the standard 1C documentation format with description, parameters, and return value:\n\n" //$NON-NLS-1$
            + "```bsl\n%s\n```\n\n" //$NON-NLS-1$
            + "Generate only the documentation comment block that should be placed above the method."; //$NON-NLS-1$

    @Override
    public Object execute(ExecutionEvent event) throws ExecutionException {
        LOG.info("execute: обработчик GenerateDocComments вызван"); //$NON-NLS-1$
        ...
        // Format the prompt
        String prompt = String.format(DOC_COMMENTS_PROMPT_TEMPLATE, selectedCode);
